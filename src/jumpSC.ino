 ...
 *  ボス（24×24ビットマップ）
 * ========================= */
const uint8_t PROGMEM BOSS_BMP[72] = {
  0x00,0x3c,0x00,0x00,0xc3,0x00,0x01,0x24,
  0x80,0x02,0x24,0x40,0x02,0x18,0x40,0x03,
  0x99,0xc0,0x04,0x81,0x20,0xe4,0x7e,0x25,
  0xe8,0x00,0x12,0xe9,0x81,0x92,0x09,0x81,
  0x92,0x71,0x81,0x8e,0x88,0x00,0x11,0x88,
  0x00,0x11,0x80,0x42,0x01,0x88,0x3c,0x11,
  0x76,0x00,0x6e,0x01,0x81,0x80,0x06,0x3e,
  0x60,0x08,0x42,0x10,0x10,0x24,0x08,0x10,
  0x24,0x08,0x08,0x42,0x10,0x0f,0x81,0xe0,
};
#define BOSS_W 24
#define BOSS_H 24
...
const uint8_t G2_START_ENEMIES   = 3;  // ゲーム開始時の敵の上限数
...
static uint8_t g2_effectiveMaxEnemies(){
  uint16_t score = g2_score < G2_BOSS_SPAWN_SCORE ? g2_score : G2_BOSS_SPAWN_SCORE;
  return G2_START_ENEMIES + (uint8_t)((G2_MAX_ENEMIES - G2_START_ENEMIES) * (uint32_t)score / G2_BOSS_SPAWN_SCORE);
}
...
void g2_update(){
  ...
  if (!g2_bossSpawned || !g2_boss.alive){
    if (now - g2_lastSpawn >= g2_spawnInterval){
      g2_lastSpawn = now;
      uint8_t effMax = g2_effectiveMaxEnemies();
      if (g2_countAlive() < effMax){
        g2_spawnEnemy();
        ...
        if (random(0,100) < extraChance && g2_countAlive() < effMax) g2_spawnEnemy();
      }
    }
  }
